// Porsche 911 (992) GT3 RS · Interactive 3D Studio — static port for farissuhail.com
// Ported from the React/Vite studio (studio-scene.ts, studio-effects.ts, model-tools.ts, studio.tsx).
...
const PAINTS = [
  { name: 'White / red', body: '#f5f5f2', wheel: '#ed001c', decal: '#ed001c' },
  { name: 'Blue / white', body: '#168dde', wheel: '#eeeeee', decal: '#eeeeee' },
  { name: 'Grey / red', body: '#777e80', wheel: '#c81024', decal: '#c81024' },
  { name: 'Purple / silver / gold', body: '#6337a0', wheel: '#c4c7ce', decal: '#e5b725' },
  { name: 'Yellow / bronze', body: '#f3d32d', wheel: '#9c8469', decal: '#151719' },
...
/* ───────────────────────── model tools ───────────────────────── */
/** Separate disconnected badge geometry while retaining every vertex attribute and transform. */
function splitConnectedGeometry(source) {
...
function createContactShadow(scene, renderer) {
  ...
  return {
    ...
    update(vehicle) {
      ...
      vehicle.traverse(object => { if (object.userData.component === 'glass') { visibility.set(object, object.visible); object.visible = false; } });
...
/* ───────────────────────── scene ───────────────────────── */
function createStudio(host, callbacks) {
  ...
  function reset() { camera.position.set(6.3, 3.4, 7.3); controls.target.set(0, .75, 0); controls.update(); dirty = true; }
  reset();
  ...
  const shadow = createContactShadow(scene, renderer);
  ...
  const draco = new DRACOLoader().setDecoderPath(`${ASSET_ROOT}draco/`).setWorkerLimit(2);
  ...
  function applyEnvironment(id) {
    ...
    if (!promise) {
      promise = new HDRLoader().loadAsync(`${ASSET_ROOT}environments/${ENVIRONMENT_FILES[id]}`).then(texture => {
  ...
  function alignTyres() {
    ...
    vehicle.traverse(object => {
      if (!object.isMesh) return;
      const materials = Array.isArray(object.material) ? object.material : [object.material];
      if (!materials.some(material => material.name === 'Tyre')) return;
    ...
    vehicle.traverse(object => {
      if (!object.isMesh || object.userData.component !== 'wheels') return;
      const center = new THREE.Box3().setFromObject(object).getCenter(new THREE.Vector3());
  ...
  const load = async () => {
    try {
      const gltf = await loader.loadAsync(`${ASSET_ROOT}porsche-gt3-rs.glb`, event => callbacks.onProgress(event.total ? Math.min(99, Math.round(event.loaded / event.total * 100)) : 0));
      ...
      vehicle.position.set(-center.x, -bounds.min.y, -center.z); vehicle.updateMatrixWorld(true); alignTyres();
      const counts = { all: 0, body: 0, glass: 0, wheels: 0, cabin: 0, trim: 0 };
      vehicle.traverse(object => {
        if (!object.isMesh) return;
        const mesh = object;
        const group = groupOf(mesh); mesh.userData.component = group;
        const original = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
        const records = original.map(source => {
          ...
          return { material, color: material.color.clone(), metalness: material.metalness ?? 0, emissive: material.emissive?.clone() ?? new THREE.Color(), emissiveIntensity: material.emissiveIntensity ?? 0, role: materialRole(mesh, material) };
        });
        mesh.material = Array.isArray(mesh.material) ? records.map(record => record.material) : records[0].material;
        resources.geometries.add(mesh.geometry);
        const meshCenter = new THREE.Box3().setFromObject(mesh).getCenter(new THREE.Vector3());
        const worldOffset = partOffset(group, meshCenter, mesh.userData.wheelRole);
        const parent = mesh.parent;
        const offset = parent.worldToLocal(meshCenter.clone().add(worldOffset)).sub(parent.worldToLocal(meshCenter.clone()));
        parts.push({ mesh, group, center: meshCenter, offset, base: mesh.position.clone(), materials: records });
        counts[group]++; counts.all++;
      });
      attachBadges();
      ...
      if (!disposed) { console.error('GT3 RS model load failed', error); callbacks.onError('The GT3 RS model could not load. Please reload the model.'); }
    }
  };
  ...
  function applyState() {
    ...
    const paint = PAINTS[state.paint] ?? PAINTS[0];
    for (const part of parts) {
      part.mesh.visible = state.group === 'all' || part.group === state.group;
      for (const record of part.materials) {
        const material = record.material;
        material.color.copy(record.color); material.metalness = record.metalness;
        if (state.paint !== 0) {
          if (record.role === 'body') material.color.set(paint.body);
          if (record.role === 'rim') { material.color.set(paint.wheel); material.metalness = state.paint === 1 ? .25 : .72; }
  ...
  const pointerUp = event => {
    ...
    const hit = raycaster.intersectObjects(parts.filter(part => part.mesh.visible).map(part => part.mesh), false)[0];
    if (hit) { const group = groupOf(hit.object); callbacks.onSelect(state.group === group ? 'all' : group); }
  };
...
/* ───────────────────────── UI (vanilla DOM port of studio.tsx) ───────────────────────── */
const ICONS = {
...
function mountStudio(rootEl) {
  const html = String.raw;
  rootEl.innerHTML = html`
    <div class="gt3-viewport" tabindex="0" role="region" aria-label="Interactive Porsche 911 GT3 RS model. Drag to orbit, scroll to zoom. Arrow keys rotate, plus and minus zoom, Home resets."></div>
    <header class="gt3-heading">
      <div class="gt3-wordmark">PORSCHE</div>
      <h1>911 GT3 <span>RS</span></h1>
      <p>992 · Interactive 3D Studio</p>
    </header>
  ...
  // Build lists
  partsNav.innerHTML = GROUPS.map(g => `<button type="button" data-group="${g.id}" disabled><span><strong>${g.name}</strong><small>${g.subtitle}</small></span><span class="gt3-count" data-count="${g.id}">—</span></button>`).join('');
  paintsEl.innerHTML = PAINTS.map((p, i) => `<button type="button" data-paint="${i}" title="${p.name}" aria-label="${p.name}" style="background-color:${p.body}" disabled><span class="gt3-check" style="color:${i === 0 || i === 4 ? '#17232f' : '#fff'}">${ICONS.check}</span></button>`).join('');
...
if (root) {
  ...
  if (supportsWebGL) window.gt3Studio = mountStudio(root);
  else root.innerHTML = '<div class="gt3-error" role="alert"><p>This browser cannot display WebGL, so the 3D studio cannot start. Try a current desktop or mobile browser with hardware acceleration enabled.</p></div>';
}
